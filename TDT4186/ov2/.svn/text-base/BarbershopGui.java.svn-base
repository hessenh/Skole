...
public class BarbershopGui extends JFrame implements Constants, Gui, ChangeListener {
	...
	private void quit() {
		...
		for(int i = 0; i < NOF_BARBERS; i++) {
			...
			totalServed += barbers[i].getNumServed();
			System.out.println("Barberer " + i + " served " + barbers[i].getNumServed() + " custmers");
		}
		System.out.println("Total customers served " + totalServed);
		System.out.println(Cashier.getInstance().getTodaysTotal());
		System.exit(0);
	}
	...
	public static void main(String args[]) {
		...
		RandomControls rc = new RandomControls();
		rc.setVisible(true);
		gui.startSimulation();
	}
}
